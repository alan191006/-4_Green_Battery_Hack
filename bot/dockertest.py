...
if __name__ == "__main__":
    ...
    client.images.build(
        path="./bot", tag=docker_image_tag, platform=get_default_platform(), quiet=False
    )
    ...
    if os.path.exists(os.path.join(data_dir, output_file)):
        print(
            f"Policy run successfully, output can be found at: {os.path.join(data_dir, output_file)}"
        )
    else:
        raise Exception(
            f"Policy run failed, no output file found at {os.path.join(data_dir, output_file)}"
        )

...
class ReserveNoGateRNNModel(pl.LightningModule):
    def __init__(
        self,
        battery: BatteryEnv,
        input_size: int,
        ...
        reserve_level: float = 0.2,
        reserve_penalty_weight: float = 1.0,
        reserve_penalty_power: float = 2.0,
    ):
        super().__init__()
        ...
        self.fc = nn.Sequential(
            nn.Linear(hidden_size + 1, fc_size),
            nn.ReLU(),
        ...
        self.reserve_penalty_weight = reserve_penalty_weight
        self.reserve_penalty_power = reserve_penalty_power
    ...
    def forward(
        ...
        for i in range(x.size(1)):
            ...
            z_t = torch.cat(
                [z_t, battery_state[:, None, :] / self.battery.capacity_kWh], dim=-1
            )
            out = self.fc(z_t)
    ...
    def reserve_penalty(self, battery_states):
        penalty = (
            torch.where(
                battery_states < self.reserve_level * self.battery.capacity_kWh,
                (self.reserve_level * self.battery.capacity_kWh - battery_states)
                ** self.reserve_penalty_power,
                0,
            )
            .sum(-1)
            .mean()
    ...
    def training_step(
        ...
        loss += self.reserve_penalty_weight * self.reserve_penalty(battery_states)
        self.log(
            "train_loss",
            loss,
            on_step=True,

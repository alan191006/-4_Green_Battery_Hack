...
def to_signature_features(dat, depth=3, downscale=2):
    sigs = signatory.signature(
        time_aug(dat[..., :-4]) / downscale, depth, stream=True, basepoint=True
    )
    combined = torch.cat((dat, sigs), dim=-1)
    return combined
...
def load_and_scale_data(
    ...
    include_pos_code: bool = False,
    include_peak_indicator: bool = False,
) -> Tuple[np.ndarray, np.ndarray, np.ndarray, BaseEstimator]:
    if data_path is not None:
        dat = pd.read_csv(data_path, parse_dates=["timestamp"])[active_columns]
    elif df is not None:
        dat = df[active_columns].copy()
    else:
    ...
    if scaler is None:
        ...
        scaler.fit(dat[[x for x in active_columns if x not in ["timestamp"]]])
    # make symmetric
    dat["price"] = np.power(dat["price"] - dat["price"].min(), 1 / 4)
    # scale data
    res = scaler.transform(dat[[x for x in active_columns if x not in ["timestamp"]]])
    # combine features
    if include_pos_code:
        hour = dat["timestamp"].dt.hour
        weekday = dat["timestamp"].dt.weekday
        ...
        res = np.concatenate([res, pos_code_day, pos_code_week], axis=1)
    if include_peak_indicator:
        peak_indicator = get_peak_indicator(dat["timestamp"]).numpy()
        res = np.concatenate([res, peak_indicator], axis=1)
    return res, price, pv_power, scaler


def learn_scaling_params(data_path: str = None, df: pd.DataFrame = None) -> np.ndarray:
    if data_path is not None:
        dat = pd.read_csv(data_path, parse_dates=["timestamp"])[active_columns]
    elif df is not None:
        dat = df[active_columns].copy()

...
class VariationNoGateRNNModel(pl.LightningModule):
    def __init__(
        self,
        battery: BatteryEnv,
        input_size: int,
        ...
        variation_penalty_weight: float = 1.0,
        variation_p: float = 1.0,
    ):
        super().__init__()
        ...
        self.variation_penalty_weight = variation_penalty_weight
        self.variation_p = variation_p
    ...
    def variation_penalty(self, battery_states):
        # penalise total variation
        if self.variation_penalty_weight > 0:
            penalty = (
                battery_states.diff(1, dim=1).abs().sum(1).pow(self.variation_p).mean()
            )
            tag = "train" if self.training else "val"
            self.log(
                f"{tag}_variation_penalty",
                penalty,
                on_step=False,
                on_epoch=True,
                prog_bar=True,
    ...
    def training_step(
        ...
        loss += self.variation_penalty_weight * self.variation_penalty(battery_states)
        self.log(
            "train_loss",
            loss,
            on_step=True,

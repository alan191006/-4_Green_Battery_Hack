...
class IfItsProfitableItsProfitable(Policy):
    def __init__(
        self,
        short_window: int = 32,
    ...
    def act(self, external_state, internal_state):
        ...
        if len(self.short_prices) == self.short_window:
            short_ema = self.calculate_ema(self.short_prices, self.short_alpha)
            medium_ema = self.calculate_ema(self.medium_prices, self.medium_alpha)
            long_ema = self.calculate_ema(self.long_prices, self.long_alpha)
...
if __name__ == "__main__":
    ...
    best_param_dict, best_profit = optimize(EMAPolicy, params)

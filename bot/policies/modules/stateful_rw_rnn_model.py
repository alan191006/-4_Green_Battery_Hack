...
class StatefulRWRNNModel(pl.LightningModule):
    def __init__(
        self,
        battery: BatteryEnv,
        input_size: int,
        ...
        self.sig_to_hold_gate = sig_to_hold_gate
        ...
        if sig_to_hold_gate:
            self.hold_gate = nn.Sequential(
                nn.Linear(2 + self.sig_channels * len(self.history_windows), fc_size),
                nn.ReLU(),
    ...
    def forward(
        ...
        path = signatory.Path(x_critical, self.sig_depth, basepoint=x_critical[:, 0, :])
    ...
    def step_forward(
        ...
        if path is None:
            path = signatory.Path(
                x_critical_t, self.sig_depth, basepoint=x_critical_t[:, 0, :]
            )
        else:
        ...
        grid_action = F.tanh(grid_action) * a_t
        pv_action = F.sigmoid(pv_action)
        return grid_action, pv_action, battery_state, h0, path
